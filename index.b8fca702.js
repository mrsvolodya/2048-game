...
var _game2048Js = require("./Game2048.js");
const game = new (0, _game2048Js.Game2048)();
...
function addLayoutOnHtml() {
    for(let row = 0; row < size; row++)for(let cell = 0; cell < size; cell++){
        if (gameField[row][cell] === 2048) winner.classList.remove("hidden");
        const setFieldHtml = fieldHtml[row].children[cell];
        const currentValue = setFieldHtml.textContent = gameField[row][cell] || "";
...
function addRandomNumber() {
    ...
    if (gameField[row][cell] === 0) gameField[row][cell] = Math.random() < 0.1 ? 4 : 2;
...
function checkGameOver() {
    ...
    let hasMoveUD = false;
    let comeIn = false;
    const result = copyGameFiel.every((r)=>copyGameFiel.every((_, i)=>r[i] > 0));
    if (result) {
        comeIn = true;
    ...
    if (!hasMoveRL && !hasMoveUD && comeIn) loseMessage.classList.remove("hidden");
}
...
},{"./Game2048.js":"8bNPT"}],"8bNPT":[function(require,module,exports) {
/* eslint-disable function-paren-newline */ var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "Game2048", ()=>Game2048);
class Game2048 {
    constructor(size = 4){
        this.size = size;
        this.score = 0;
